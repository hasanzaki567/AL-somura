...
import { X, Check, Sparkles, Shield, Truck, ShoppingBag, Upload, Sliders, Maximize2 } from 'lucide-react';
...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const currentImage = product.colors.length > 0 && selectedColor.image
    ? selectedColor.image
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#090100]/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        {useFabricCanvas ? (
          ...
          <div className="w-full aspect-[4/3] sm:aspect-square relative overflow-hidden rounded-lg bg-white p-2 flex items-center justify-center shadow-inner">
            <img
              src={currentImage}
              alt={product.name}
          ...
          {/* Alternate thumbnails if multiple */}
              ...
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`w-14 h-14 rounded-md border overflow-hidden p-0.5 bg-white cursor-pointer transition-all ${
                    selectedImageIndex === idx ? 'border-[#825425] ring-2 ring-[#825425]/40' : 'border-transparent opacity-70 hover:opacity-100'
                  }`}
                >
                  <img src={img} alt="Thumbnail" referrerPolicy="no-referrer" className="w-full h-full object-cover rounded" />
                </button>
              ))}
            </div>
          )}
